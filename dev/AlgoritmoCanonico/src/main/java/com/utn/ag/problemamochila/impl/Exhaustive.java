...
public class Exhaustive {
	...
	public void fillBag(Bag mochila, House casa) {
		...
		processValueVolumeProp(casa.items);  //No es Necesario para el Exahustivo pero auda a visualizar el informe final
		...
		for (int index = 1; index < casa.items.size(); index++) { 
			...
			for (ICombinatoricsVector<Item> combination : gen) {
				if (fitsInBag((ArrayList<Item>)combination.getVector(), mochila)){ //Verifica que puedan entrar en la mochila antes de agregarlas a la combinacion
