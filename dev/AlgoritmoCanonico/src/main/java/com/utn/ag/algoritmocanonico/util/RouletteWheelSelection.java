...
public class RouletteWheelSelection {
	...
	/**
	 * Selects the required number of candidates from the population with the
	 * probability of selecting any particular candidate being proportional to
	 * that candidate's fitness score. Selection is with replacement (the same
	 * candidate may be selected multiple times).
	 * 
	 * @param <S>
	 *            The type of the evolved objects in the population.
	 * @param population
	 *            The candidates to select from.
	 * @param naturalFitnessScores
	 *            True if higher fitness scores indicate fitter individuals,
	 *            false if lower fitness scores indicate fitter individuals.
	 * @param selectionSize
	 ...
	 */
	public static Poblacion select(Poblacion population, boolean naturalFitnessScores,
		...
		cumulativeFitnesses[0] = getAdjustedFitness(population.get(0)
				.getFitness(), naturalFitnessScores);
		for (int i = 1; i < population.size(); i++) {
			double fitness = getAdjustedFitness(population.get(i).getFitness(),
					naturalFitnessScores);
			cumulativeFitnesses[i] = cumulativeFitnesses[i - 1] + fitness;
		}
		Poblacion selection = new Poblacion(selectionSize);
	...
	public static Poblacion select(Poblacion population) {
		return select(population, true, population.size(), new Random());
	}
